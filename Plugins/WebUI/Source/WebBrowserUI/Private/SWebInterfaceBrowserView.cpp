...
SWebInterfaceBrowserView::~SWebInterfaceBrowserView()
{
	if (BrowserWindow.IsValid())
	{
		...
		if (BrowserWindow->OnDragBeginSelf().IsBoundToObject(this))
		{//wqq 2021.1.8 
			BrowserWindow->OnDragBeginSelf().Unbind();
		}

		if (BrowserWindow->OnDragProdToScene().IsBoundToObject(this))
		{//wqq 2021.1.9 
			BrowserWindow->OnDragProdToScene().Unbind();
		}
	}
...
void SWebInterfaceBrowserView::Construct(const FArguments& InArgs, const TSharedPtr<IWebInterfaceBrowserWindow>& InWebBrowserWindow)
{
	...
	OnDragBeginSelf = InArgs._OnDragBeginSelf;  //wqq 2021.1.8 
	OnDragProdToScene = InArgs._OnDragProdToScene;  //wqq 2021.1.9 
	...
	if (BrowserWindow.IsValid())
	{
		...
		if (!BrowserWindow->OnDragBeginSelf().IsBound())
		{//wqq 2021.1.8 
			BrowserWindow->OnDragBeginSelf().BindSP(this, &SWebInterfaceBrowserView::HandleDragBeginSelf);
		}
		else
		{
			check(!OnDragBeginSelf.IsBound());
		}
		if (!BrowserWindow->OnDragProdToScene().IsBound())
		{//wqq 2021.1.9 
			BrowserWindow->OnDragProdToScene().BindSP(this, &SWebInterfaceBrowserView::HandleDragProdToScene);
		}
		else
		{
			check(!OnDragProdToScene.IsBound());
		}
...
bool SWebInterfaceBrowserView::HandleDragBeginSelf(FString URL)
{//wqq 2021.1.8 
	if (OnDragBeginSelf.IsBound())
	{
		return OnDragBeginSelf.Execute(URL);
	}

	return false;
}

bool SWebInterfaceBrowserView::HandleDragProdToScene(FString URL)
{//wqq 2021.1.9
	if (OnDragProdToScene.IsBound())
	{
		return OnDragProdToScene.Execute(URL);
	}
