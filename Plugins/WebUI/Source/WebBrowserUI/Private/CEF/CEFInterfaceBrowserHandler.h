...
public:
	...
	{//wqq 2021.1.8 拖拽开始
		...
		browser->GetHost()->DragTargetDragEnter(drag_data, mouse_event, DRAG_OPERATION_EVERY);  //发送这个消息后，cef库会调用CefDragHandle处理拖拽事件
		return true;
	}
	virtual void UpdateDragCursor(CefRefPtr<CefBrowser> browser,
		DragOperation operation)
	{//wqq 2021.1.8 更新拖拽过程中的鼠标
		CefMouseEvent m;
		m.modifiers = EVENTFLAG_LEFT_MOUSE_BUTTON;
		browser->GetHost()->DragTargetDragOver(m, DRAG_OPERATION_EVERY); 
	}
#if !PLATFORM_LINUX
...
public:
	...
	virtual bool OnDragEnter(CefRefPtr<CefBrowser> browser,
		...
		cef_drag_operations_mask_t mask) override;   //wqq 2021.1.8 拖拽事件
public:
	...
	IWebInterfaceBrowserWindow::FOnDragBeginSelfDelegate& OnDragBeginSelf()
	{
		return DragBeginSelfDelegate;  //wqq 2021.1.8
	}
	IWebInterfaceBrowserWindow::FOnDragProdToSceneDelegate& OnDragProdToScene()
	{
		return DragProdToSceneDelegate;  //wqq 2021.1.9
	}
...
private:
	...
	IWebInterfaceBrowserWindow::FOnDragBeginSelfDelegate  DragBeginSelfDelegate;  //wqq 2021.1.8 
	IWebInterfaceBrowserWindow::FOnDragProdToSceneDelegate  DragProdToSceneDelegate;  //wqq 2021.1.9 
