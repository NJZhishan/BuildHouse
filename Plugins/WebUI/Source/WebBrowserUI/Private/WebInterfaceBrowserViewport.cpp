...
FReply FWebInterfaceBrowserViewport::OnMouseButtonDown(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
	...
	if (MouseEvent.GetEffectingButton() == EKeys::LeftMouseButton)
	{
		...
		if (Path->IsValid())
		{
			...
			Reply = Reply.CaptureMouse(TopWidget);
		}
	}
	if(1)
	{//wqq 2021.1.11 鼠标左键按下时判断是否有拖拽事件
		const FWidgetPath* Path = MouseEvent.GetEventPath();
		if (Path->IsValid())
		{
			TSharedRef<SWidget> TopWidget = Path->Widgets.Last().Widget;
			return Reply.DetectDrag(TopWidget, EKeys::LeftMouseButton);
		}
	}
	return Reply;
}
...
{
	FReply reply = WebBrowserWindow->OnMouseMove(MyGeometry, MouseEvent, bIsPopup);
	return reply;
}
