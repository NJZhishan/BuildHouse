...
TSharedRef<SWidget> UWebInterface::RebuildWidget()
{
	...
	WebInterfaceWidget = SNew(SWebInterface)
		...
		.OnDragBeginSelf(BIND_UOBJECT_DELEGATE(FOnDragBeginSelfDelegate, HandleDragBegin))  //wqq 2021.1.8 开始拖拽
	    .OnDragProdToScene(BIND_UOBJECT_DELEGATE(FOnDragProdToSceneDelegate, HandleDragProdToScene));  //wqq 2021.1.9 拖拽模型到场景中 
...
//wqq 2021.1.8 
bool UWebInterface::HandleDragBegin(FString dragData)
{
	OnDragBeginSelfEvent.Broadcast(dragData);
	return false;
}

//wqq 2021.1.9 拖拽模型到场景中 
bool UWebInterface::HandleDragProdToScene(FString dragData)
{
	OnDragProdToScene.Broadcast(dragData);
	return false;
}
