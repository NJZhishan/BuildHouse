	...
	const CefString& FailedUrl)
{
	...
	if (InErrorCode == ERR_ABORTED)  //wqq 2021.1.8 
		return;
	...
	bool IsRedirect)
{
	...
	//wqq 2021.1.8 如果是点击的素材，不能继续加载页面
	CefString str = Request->GetURL();  //获得即将打开的页面url
	cef_uri_unescape_rule_t rul = UU_SPACES;
	str = CefURIDecode(str, true, rul);   
	std::wstring transStr = str.ToWString();
	FString newUrl(transStr.c_str());
	if (newUrl.Contains("material/$["))
		return true;  //返回true表示停止加载页面，返回false表示继续加载页面
	return false;
}
...
bool FCEFInterfaceBrowserHandler::OnDragEnter(CefRefPtr<CefBrowser> browser,
	CefRefPtr<CefDragData> dragData,
	cef_drag_operations_mask_t mask)
{ //wqq 2022.1.8 拖拽
	CefString str = dragData->GetLinkURL();
	cef_uri_unescape_rule_t rul = UU_SPACES;
	str = CefURIDecode(str, true, rul);
	std::wstring transStr = str.ToWString();
	FString newUrl(transStr.c_str());
	if(OnDragBeginSelf().IsBound())
		OnDragBeginSelf().Execute(newUrl);
	return false;
}
