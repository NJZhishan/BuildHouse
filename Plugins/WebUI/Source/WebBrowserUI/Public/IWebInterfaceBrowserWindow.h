...
public:
	...
	/** A delegate that is invoked when drag begin. */
	DECLARE_DELEGATE_RetVal_OneParam(bool,FOnDragBeginSelfDelegate, FString);
	virtual FOnDragBeginSelfDelegate& OnDragBeginSelf() = 0;  //wqq 2021.1.8 

	/** A delegate that is invoked when linetrace. */
	DECLARE_DELEGATE_RetVal_OneParam(bool, FOnDragProdToSceneDelegate, FString);
	virtual FOnDragProdToSceneDelegate& OnDragProdToScene() = 0;  //wqq 2021.1.9 
